...
import numpy as np
from source.lp_solver import solve_lp
from source.lp_with_reg import solve_bat
...
class BatteryController:
    ...
    def __init__(self, owner_id, b_max, b_min, eff_c, eff_d, d_max, d_min):
        """
    ...
    def find_optimal_step(self, fload, fprice_b, fprice_s, commitment=None):
        """
        ...
        #print(res[1])
        xs = res[2]
        #print(len(fload), res[0])
        if res[0] == 'optimal':
            return xs
        else:
            print('Problem not optimal', self)
            return xs
